...
public:
    ...
    // Copy a 3-D vector
    //
    inline btVector3( const btVector3 & vec );

    // Construct a 3-D vector from x, y, and z elements
    //
    inline btVector3( float x, float y, float z );

    // Copy elements from a 3-D point into a 3-D vector
    //
    explicit inline btVector3( const Point3 & pnt );

    // Set all elements of a 3-D vector to the same scalar value
    //
    explicit inline btVector3( float scalar );

    // Assign one 3-D vector to another
    //
    inline btVector3 & operator =( const btVector3 & vec );

    // Set the x element of a 3-D vector
    //
    inline btVector3 & setX( float x );

    // Set the y element of a 3-D vector
    //
    inline btVector3 & setY( float y );

    // Set the z element of a 3-D vector
    //
    inline btVector3 & setZ( float z );
    ...
    //
    inline btVector3 & setElem( int idx, float value );
    ...
    // Add two 3-D vectors
    //
    inline const btVector3 operator +( const btVector3 & vec ) const;
    ...
    //
    inline const btVector3 operator *( float scalar ) const;
    ...
    //
    inline btVector3 & operator *=( float scalar );
    ...
    //
    inline const btVector3 operator -( ) const;

    // Construct x axis
    //
    static inline const btVector3 xAxis( );

    // Construct y axis
    //
    static inline const btVector3 yAxis( );

    // Construct z axis
    //
    static inline const btVector3 zAxis( );
...
// Multiply a 3-D vector by a scalar
//
inline const btVector3 operator *( float scalar, const btVector3 & vec );

// Multiply two 3-D vectors per element
//
inline const btVector3 mulPerElem( const btVector3 & vec0, const btVector3 & vec1 );

// Divide two 3-D vectors per element
// NOTE:
// Floating-point behavior matches standard library function divf4.
//
inline const btVector3 divPerElem( const btVector3 & vec0, const btVector3 & vec1 );

// Compute the reciprocal of a 3-D vector per element
// NOTE:
// Floating-point behavior matches standard library function recipf4.
//
inline const btVector3 recipPerElem( const btVector3 & vec );

// Compute the square root of a 3-D vector per element
// NOTE:
// Floating-point behavior matches standard library function sqrtf4.
//
inline const btVector3 sqrtPerElem( const btVector3 & vec );
...
//
inline const btVector3 rsqrtPerElem( const btVector3 & vec );

// Compute the absolute value of a 3-D vector per element
//
inline const btVector3 absPerElem( const btVector3 & vec );

// Copy sign from one 3-D vector to another, per element
//
inline const btVector3 copySignPerElem( const btVector3 & vec0, const btVector3 & vec1 );

// Maximum of two 3-D vectors per element
//
inline const btVector3 maxPerElem( const btVector3 & vec0, const btVector3 & vec1 );

// Minimum of two 3-D vectors per element
//
inline const btVector3 minPerElem( const btVector3 & vec0, const btVector3 & vec1 );

// Maximum element of a 3-D vector
//
inline float maxElem( const btVector3 & vec );

// Minimum element of a 3-D vector
//
inline float minElem( const btVector3 & vec );

// Compute the sum of all elements of a 3-D vector
//
inline float sum( const btVector3 & vec );

// Compute the dot product of two 3-D vectors
//
inline float dot( const btVector3 & vec0, const btVector3 & vec1 );

// Compute the square of the length of a 3-D vector
//
inline float lengthSqr( const btVector3 & vec );

// Compute the length of a 3-D vector
//
inline float length( const btVector3 & vec );

// Normalize a 3-D vector
// NOTE:
// The result is unpredictable when all elements of vec are at or near zero.
//
inline const btVector3 normalize( const btVector3 & vec );

// Compute cross product of two 3-D vectors
//
inline const btVector3 cross( const btVector3 & vec0, const btVector3 & vec1 );

// Outer product of two 3-D vectors
//
inline const Matrix3 outer( const btVector3 & vec0, const btVector3 & vec1 );

// Pre-multiply a row vector by a 3x3 matrix
//
inline const btVector3 rowMul( const btVector3 & vec, const Matrix3 & mat );

// Cross-product matrix of a 3-D vector
//
inline const Matrix3 crossMatrix( const btVector3 & vec );

// Create cross-product matrix and multiply
// NOTE:
// Faster than separately creating a cross-product matrix and multiplying.
//
inline const Matrix3 crossMatrixMul( const btVector3 & vec, const Matrix3 & mat );

// Linear interpolation between two 3-D vectors
// NOTE:
// Does not clamp t between 0 and 1.
//
inline const btVector3 lerp( float t, const btVector3 & vec0, const btVector3 & vec1 );
...
// Does not clamp t between 0 and 1.
//
inline const btVector3 slerp( float t, const btVector3 & unitVec0, const btVector3 & unitVec1 );

// Conditionally select between two 3-D vectors
//
inline const btVector3 select( const btVector3 & vec0, const btVector3 & vec1, bool select1 );

// Load x, y, and z elements from the first three words of a float array.
//
//
inline void loadXYZ( btVector3 & vec, const float * fptr );
...
//
inline void storeXYZ( const btVector3 & vec, float * fptr );

// Load three-half-floats as a 3-D vector
// NOTE:
// This transformation does not support either denormalized numbers or NaNs.
//
inline void loadHalfFloats( btVector3 & vec, const unsigned short * hfptr );
...
//
inline void storeHalfFloats( const btVector3 & vec, unsigned short * hfptr );
...
//
inline void print( const btVector3 & vec );

// Print a 3-D vector and an associated string identifier
// NOTE:
// Function is only defined when _VECTORMATH_DEBUG is defined.
//
inline void print( const btVector3 & vec, const char * name );
...
public:
    ...
    //
    inline Vector4( const btVector3 & xyz, float w );

    // Copy x, y, and z from a 3-D vector into a 4-D vector, and set w to 0
    //
    explicit inline Vector4( const btVector3 & vec );
    ...
    //
    inline Vector4 & setXYZ( const btVector3 & vec );

    // Get the x, y, and z elements of a 4-D vector
    //
    inline const btVector3 getXYZ( ) const;
...
public:
    ...
    //
    explicit inline Point3( const btVector3 & vec );
    ...
    //
    inline const btVector3 operator -( const Point3 & pnt ) const;

    // Add a 3-D point to a 3-D vector
    //
    inline const Point3 operator +( const btVector3 & vec ) const;
    ...
    // Perform compound assignment and addition with a 3-D vector
    //
    inline Point3 & operator +=( const btVector3 & vec );
...
//
inline const Point3 scale( const Point3 & pnt, const btVector3 & scaleVec );

// Scalar projection of a 3-D point on a unit-length 3-D vector
//
inline float projection( const Point3 & pnt, const btVector3 & unitVec );
...
public:
    ...
    //
    inline Quat( const btVector3 & xyz, float w );
    ...
    // This function does not change the w element.
    //
    inline Quat & setXYZ( const btVector3 & vec );
    ...
    //
    static inline const Quat rotation( const btVector3 & unitVec0, const btVector3 & unitVec1 );

    // Construct a quaternion to rotate around a unit-length 3-D vector
    //
    static inline const Quat rotation( float radians, const btVector3 & unitVec );
...
//
inline const btVector3 rotate( const Quat & unitQuat, const btVector3 & vec );
...
public:
    ...
    //
    inline Matrix3( const btVector3 & col0, const btVector3 & col1, const btVector3 & col2 );
    ...
    //
    inline Matrix3 & setCol0( const btVector3 & col0 );

    // Set column 1 of a 3x3 matrix
    //
    inline Matrix3 & setCol1( const btVector3 & col1 );

    // Set column 2 of a 3x3 matrix
    //
    inline Matrix3 & setCol2( const btVector3 & col2 );

    // Get column 0 of a 3x3 matrix
    //
    inline const btVector3 getCol0( ) const;

    // Get column 1 of a 3x3 matrix
    //
    inline const btVector3 getCol1( ) const;

    // Get column 2 of a 3x3 matrix
    //
    inline const btVector3 getCol2( ) const;

    // Set the column of a 3x3 matrix referred to by the specified index
    //
    inline Matrix3 & setCol( int col, const btVector3 & vec );

    // Set the row of a 3x3 matrix referred to by the specified index
    //
    inline Matrix3 & setRow( int row, const btVector3 & vec );

    // Get the column of a 3x3 matrix referred to by the specified index
    //
    inline const btVector3 getCol( int col ) const;

    // Get the row of a 3x3 matrix referred to by the specified index
    //
    inline const btVector3 getRow( int row ) const;

    // Subscripting operator to set or get a column
    //
    inline btVector3 & operator []( int col );

    // Subscripting operator to get a column
    //
    inline const btVector3 operator []( int col ) const;
    ...
    //
    static inline const Matrix3 rotationZYX( const btVector3 & radiansXYZ );

    // Construct a 3x3 matrix to rotate around a unit-length 3-D vector
    //
    static inline const Matrix3 rotation( float radians, const btVector3 & unitVec );
    ...
    //
    static inline const Matrix3 scale( const btVector3 & scaleVec );
...
//
inline const Matrix3 appendScale( const Matrix3 & mat, const btVector3 & scaleVec );

// Prepend (pre-multiply) a scale transformation to a 3x3 matrix
// NOTE:
// Faster than creating and multiplying a scale transformation matrix.
//
inline const Matrix3 prependScale( const btVector3 & scaleVec, const Matrix3 & mat );
...
public:
    ...
    //
    inline Matrix4( const Matrix3 & mat, const btVector3 & translateVec );

    // Construct a 4x4 matrix from a unit-length quaternion and a 3-D vector
    //
    inline Matrix4( const Quat & unitQuat, const btVector3 & translateVec );
    ...
    // This function does not change the bottom row elements.
    //
    inline Matrix4 & setTranslation( const btVector3 & translateVec );

    // Get the translation component of a 4x4 matrix
    //
    inline const btVector3 getTranslation( ) const;
    ...
    //
    inline const Vector4 operator *( const btVector3 & vec ) const;
    ...
    //
    static inline const Matrix4 rotationZYX( const btVector3 & radiansXYZ );

    // Construct a 4x4 matrix to rotate around a unit-length 3-D vector
    //
    static inline const Matrix4 rotation( float radians, const btVector3 & unitVec );
    ...
    //
    static inline const Matrix4 scale( const btVector3 & scaleVec );

    // Construct a 4x4 matrix to perform translation
    //
    static inline const Matrix4 translation( const btVector3 & translateVec );

    // Construct viewing matrix based on eye position, position looked at, and up direction
    //
    static inline const Matrix4 lookAt( const Point3 & eyePos, const Point3 & lookAtPos, const btVector3 & upVec );
...
// Faster than creating and multiplying a scale transformation matrix.
//
inline const Matrix4 appendScale( const Matrix4 & mat, const btVector3 & scaleVec );

// Prepend (pre-multiply) a scale transformation to a 4x4 matrix
// NOTE:
// Faster than creating and multiplying a scale transformation matrix.
//
inline const Matrix4 prependScale( const btVector3 & scaleVec, const Matrix4 & mat );
...
public:
    ...
    //
    inline Transform3( const btVector3 & col0, const btVector3 & col1, const btVector3 & col2, const btVector3 & col3 );

    // Construct a 3x4 transformation matrix from a 3x3 matrix and a 3-D vector
    //
    inline Transform3( const Matrix3 & tfrm, const btVector3 & translateVec );

    // Construct a 3x4 transformation matrix from a unit-length quaternion and a 3-D vector
    //
    inline Transform3( const Quat & unitQuat, const btVector3 & translateVec );
    ...
    // Set translation component
    //
    inline Transform3 & setTranslation( const btVector3 & translateVec );
    ...
    //
    inline Transform3 & setCol0( const btVector3 & col0 );

    // Set column 1 of a 3x4 transformation matrix
    //
    inline Transform3 & setCol1( const btVector3 & col1 );

    // Set column 2 of a 3x4 transformation matrix
    //
    inline Transform3 & setCol2( const btVector3 & col2 );

    // Set column 3 of a 3x4 transformation matrix
    //
    inline Transform3 & setCol3( const btVector3 & col3 );
    ...
    //
    inline const btVector3 getCol3( ) const;

    // Set the column of a 3x4 transformation matrix referred to by the specified index
    //
    inline Transform3 & setCol( int col, const btVector3 & vec );
    ...
    //
    static inline const Transform3 rotationZYX( const btVector3 & radiansXYZ );

    // Construct a 3x4 transformation matrix to rotate around a unit-length 3-D vector
    //
    static inline const Transform3 rotation( float radians, const btVector3 & unitVec );
    ...
    //
    static inline const Transform3 scale( const btVector3 & scaleVec );

    // Construct a 3x4 transformation matrix to perform translation
    //
    static inline const Transform3 translation( const btVector3 & translateVec );
...
// Faster than creating and multiplying a scale transformation matrix.
//
inline const Transform3 appendScale( const Transform3 & tfrm, const btVector3 & scaleVec );

// Prepend (pre-multiply) a scale transformation to a 3x4 transformation matrix
// NOTE:
// Faster than creating and multiplying a scale transformation matrix.
//
inline const Transform3 prependScale( const btVector3 & scaleVec, const Transform3 & tfrm );
